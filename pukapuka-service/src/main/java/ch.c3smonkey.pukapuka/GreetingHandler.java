...
public class GreetingHandler {
	...
	public Mono<String> getGreetingStringMono() {
		return webClient.get().uri("http://aloha-service").exchange().flatMap(resp -> resp.bodyToMono(String.class))
	...
	public Mono<String> getGreetingStringMono(String locale) {
		...
		return webClient.get().uri("http://aloha-service/" + locale).exchange().flatMap(resp -> resp.bodyToMono(String.class))
				.doOnSuccess(greeting -> LOG.log(Level.INFO, "Greeting: " +greeting));
	}
}
